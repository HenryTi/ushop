...
export function VStart({ data, me, setMeAdmin, setAdmin }: Props) {
    let { meAdmin, sysAdmins, admins } = useSnapshot(data);
    let app = getAppBase();
    let onAdmin: () => Promise<void>;
    ...
    let onAddAdmin = async (role: EnumAdminRoleInEdit) => {
        ...
        let ret = await app.fetch(app.cUser.select<Admin>(captionSelectUser));
    ...
    let renderAdminItem = (admin: Admin, index: number) => {
        ...
        return app.cUser.renderUser(id, (user: User) => {
